"""
run_pipeline_gold.py
═══════════════════════════════════════════════════════════════════════════════
MAIN ORCHESTRATOR — Adaptive Walk-Forward Supervised Learning Pipeline
GOLD (XAUUSD) — 1H Resampled Data

Usage:
    python run_pipeline_gold.py

Output directory: /Users/macos/Documents/ALGO/04_Models/walk_forward_ml/output_gold/
═══════════════════════════════════════════════════════════════════════════════
"""
...
NEWS_FILE       = None
OUTPUT_DIR      = "/Users/macos/Documents/ALGO/04_Models/walk_forward_ml/output_gold"
os.makedirs(OUTPUT_DIR, exist_ok=True)

CFG = {
    "min_train_months":       24,      # Minimum months of data before first fold
    "embargo_bars":           40,      # ~1 week of 1H bars between train / val / test
    "rolling_window_months":  48,      # Rolling window = 48 months train (≈ 4 years)
    "holdout_months":         24,      # Final 24 months locked as terminal test
...
# ─────────────────────────────────────────────────────────────────────────────
print("\n" + "═" * 80)
print("  ADAPTIVE WALK-FORWARD SUPERVISED LEARNING PIPELINE — GOLD (XAUUSD)")
print("═" * 80)
print(f"\n[1] Loading {PRIMARY_TF_FILE}...")
df_raw = load_mt5_csv(PRIMARY_TF_FILE, sep=",")
...
features_df  = build_features(df_raw, news_mask_path=None)
